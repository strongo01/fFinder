...
class _ProductEditSheetState extends State<ProductEditSheet> {
  ...
  @override
  void dispose() {
    // Hier worden ze pas opgeruimd als de sheet écht weg is
    caloriesController.dispose();
    fatController.dispose();
    amountController.dispose();
    saturatedFatController.dispose();
  ...
  Future<void> _fetchDetails() async {
    ...
    if (widget.productData != null) {
      try {
        final isFav = await _isFavorite(widget.barcode); // Zorg dat _isFavorite bereikbaar is of pas dit aan
        final nutriments = widget.productData!['nutriments_per_100g'] as Map<String, dynamic>? ?? {};
        ...
        final loadedProduct = Product(
          ...
          additives: Additives(
            [],
            (widget.productData!['additives'] as List<dynamic>?)?.map((e) => e.toString()).toList() ?? [],
          ),
          allergens: Allergens(
            [],
            (widget.productData!['allergens'] as List<dynamic>?)?.map((e) => e.toString()).toList() ?? [],
          ),
        );
    ...
    try {
      ...
      if (result.status == ProductResultV3.statusSuccess && result.product != null) {
        ...
        if (caloriesController.text.isEmpty) {
             caloriesController.text = p.nutriments?.getValue(Nutrient.energyKCal, PerSize.oneHundredGrams)?.toString() ?? '';
             fatController.text = p.nutriments?.getValue(Nutrient.fat, PerSize.oneHundredGrams)?.toString() ?? '';
             saturatedFatController.text = p.nutriments?.getValue(Nutrient.saturatedFat, PerSize.oneHundredGrams)?.toString() ?? '';
             carbsController.text = p.nutriments?.getValue(Nutrient.carbohydrates, PerSize.oneHundredGrams)?.toString() ?? '';
             sugarsController.text = p.nutriments?.getValue(Nutrient.sugars, PerSize.oneHundredGrams)?.toString() ?? '';
             fiberController.text = p.nutriments?.getValue(Nutrient.fiber, PerSize.oneHundredGrams)?.toString() ?? '';
             proteinsController.text = p.nutriments?.getValue(Nutrient.proteins, PerSize.oneHundredGrams)?.toString() ?? '';
             saltController.text = p.nutriments?.getValue(Nutrient.salt, PerSize.oneHundredGrams)?.toString() ?? '';
        }
  ...
  void _fillControllers(Map<String, dynamic> nutriments) {
    if (caloriesController.text.isNotEmpty) return; // Niet overschrijven als er al iets staat
  ...
  // LET OP: Kopieer je _isFavorite, _addFavoriteProduct, _removeFavoriteProduct, _addRecentProduct, _showAddLogDialog logica 
  // óf zorg dat ze beschikbaar zijn in deze scope. 
  // Als ze in de parent class staan, moet je ze misschien doorgeven of deze class in hetzelfde bestand zetten.
  // Voor nu ga ik ervan uit dat ze in hetzelfde bestand staan en bereikbaar zijn.
  ...
  Widget build(BuildContext context) {
    ...
    if (error != null) {
      return SizedBox(height: 200, child: Center(child: Text(error!, style: const TextStyle(color: Colors.red))));
    }
    if (product == null) {
      return const SizedBox(height: 200, child: Center(child: Text('Geen productinformatie beschikbaar.')));
    }
    ...
    return Form(
      ...
      child: SingleChildScrollView(
        padding: EdgeInsets.only(
          ...
          right: 16.0, 
          top: 16.0,
          bottom: MediaQuery.of(context).viewInsets.bottom + 16, // Belangrijk voor keyboard
        ),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          mainAxisSize: MainAxisSize.min, // Belangrijk voor modal sheet
          children: [
            Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                  ...
                  onPressed: () async {
                    if (isFavorite) {
                      ...
                      if (_formKey.currentState!.validate()) {
                        ...
                        await _addFavoriteProduct(product!, editedNutriments: nutrimentsData);
                        if(mounted) setState(() => isFavorite = !isFavorite);
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Vul alle verplichte velden in (kcal).'), backgroundColor: Colors.red));
                      }
                    }
                  },
                ),
                IconButton(
                  ...
                  onPressed: () async {
                    if (_formKey.currentState!.validate()) {
                      final nutrimentsData = _getNutrimentsFromControllers();
                      await _addRecentProduct(product!, editedNutriments: nutrimentsData);
            ...
            const SizedBox(height: 8),
            _buildEditableInfoRow('Energie (kcal)', caloriesController, isDarkMode, isRequired: true),
            _buildEditableInfoRow('Vetten', fatController, isDarkMode),
            _buildEditableInfoRow('  - Waarvan verzadigd', saturatedFatController, isDarkMode),
            _buildEditableInfoRow('Koolhydraten', carbsController, isDarkMode),
            _buildEditableInfoRow('  - Waarvan suikers', sugarsController, isDarkMode),
            _buildEditableInfoRow('Vezels', fiberController, isDarkMode),
            _buildEditableInfoRow('Eiwitten', proteinsController, isDarkMode),
            _buildEditableInfoRow('Zout', saltController, isDarkMode),
            const Divider(height: 24),
            _buildInfoRow('Additieven', product!.additives?.names.join(", ")),
            _buildInfoRow('Allergenen', product!.allergens?.names.join(", ")),
            ...
            if (widget.isForMeal) ...[
              ...
              TextFormField(
                ...
                decoration: const InputDecoration(labelText: 'Hoeveelheid voor maaltijd', suffixText: 'g', border: OutlineInputBorder()),
                validator: (value) {
                  if (value == null || value.isEmpty) return 'Hoeveelheid is verplicht';
                  if (double.tryParse(value.replaceAll(',', '.')) == null) return 'Ongeldig getal';
                  return null;
                },
              ),
              const SizedBox(height: 16),
                ...
                onPressed: () {
                  if (_formKey.currentState!.validate()) {
                    ...
                    final productForMeal = {
                      ...
                      'product_name': product!.productName,
                      'brands': product!.brands,
                      'image_front_small_url': product!.imageFrontSmallUrl,
                      'nutriments_per_100g': nutrimentsData,
                    };
                    final result = {
